...
(async () => {
  ...
  try {
    ...
    logger.info(`LOG: Found div#GridViewBooking_content`);

    // 6. Parse the table
    const bookings = [];
    const rows = await frameContent.locator('div#GridViewBooking_content table tbody tr.row').all();
    logger.info(`LOG: Found ${rows.length} booking rows in table`);
    ...
    for (const row of rows) {
      ...
      if (cells.length >= 9) {
        ...
        logger.info(`LOG: Parsed booking: ${referenceNumber} - ${roomName} on ${date} ${startTime}-${endTime}`);
      }
    }
    ...
    logger.info('✅ Bookings scraping complete. Data written to:', outputLog);
    ...
    const scrapeEndTime = Date.now();
    logger.error('❌ Bookings scraping failed:', error.message);
